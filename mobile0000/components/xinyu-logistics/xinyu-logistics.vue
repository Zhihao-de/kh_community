			...
			<view>
				<view class="flex list">
					...
					<view class="info flex1">
						<view class="title address">[收货地址]{{address.province}} {{address.city}} {{address.address}}</view>
					</view>
				</view>
				<view class="flex list" :class="{one: index == 0 && wlInfo.delivery_status == 1}" v-for="(item, index) in wlInfo.list"
				 :key="index">
					<view class="time">
						<view class="day">{{item.time.slice(0,10)}}</view>
						<view>{{item.time.slice(11,19)}}</view>
					</view>
					<view class="info flex1">
