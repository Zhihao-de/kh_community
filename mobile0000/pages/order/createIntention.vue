	...
	<view>
		...
		<navigator url="/pages/address/address?source=1" class="address-section">
			<view class="order-content">
				...
				<view v-if="name!=''" class="wrapper">
					<view class="u-box">
						<text class="name" v-model="name">{{name}}</text>
						<text class="mobile" v-model="phone">{{phone}}</text>
					</view>
					...
					<view class="g-item">
						...
						<view class="right">
							...
							<view class="price-box">
								<text class="price">￥{{item.product.retail_price}}</text>
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b">
				...
				<text class="cell-tip" v-model="amount">￥{{amount}}</text>
			</view>
...
<script>
	export default {
		...
		onShow() {
			...
			this.name = term.contact_name;
			this.phone = term.contact_phone;
		...
		onLoad(options) {
			this.wx_open_id = uni.getStorageSync("userDetail").wx_open_id;
			this.wx_union_id = uni.getStorageSync("userDetail").wx_union_id;
			this.wx_name = uni.getStorageSync("userDetail").wx_name;
			this.wx_avatar_url = uni.getStorageSync("userDetail").wx_avatar_url;
			//this.wx_mobile = uni.getStorageSync("userDetail").mobile;
		...
		methods: {
			...
			getSerialNumber() {
				...
				return "LY" + yyyyMMddHHmmss + "00001";
			...
			submit() {
				...
				//产生留言订单
				that.util.request(this.api.ApiRoot + "intentions/", {
					...
					wx_open_id: that.wx_open_id,
					wx_union_id: that.wx_union_id,
					wx_name: that.wx_name,
					wx_avatar_url: that.wx_avatar_url,
					...
					uni.showModal({
						...
						success: function(res) {
							if (res.confirm) {
								...
								uni.redirectTo({
									url:`/page/intention/intention_customer`
								})
