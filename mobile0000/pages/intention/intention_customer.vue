					...
					<view class="order-item" v-for="(item,index) in tabItem.intentionList" :key="index">
						...
						<scroll-view class="goods-box" scroll-x>
							<view v-for="(goodsItem, goodsIndex) in item.detail" :key="goodsIndex" class="goods-box-single">
								<image class="goods-img" lazy-load="true" :src="goodsItem.product.main_pic_url || '/static/missing-face.png'"
								 mode="aspectFill"></image>
								<view class="right">
									<text class="title clamp">{{goodsItem.product.name}}</text>
									<text class="attr-box">{{goodsItem.retail_price}} x {{goodsItem.quantity}} ----{{goodsItem.product.weight}}
										{{goodsItem.product.unit}} </text>
		...
		methods: {
			...
			async loadData(source) {
				...
				setTimeout(() => {
					...
					let intentionList = intention_info.filter(item => {
						//添加不同状态下订单的表现形式
						item = Object.assign(item, this.intentionflagsExp(item.info.flags));
						//演示数据所以自己进行状态筛选
						if (flags === 8) {
							//8为全部订单
							return item;
			...
			cancelIntention(item) {
				...
				new Promise(resolve => {
					uni.request({
						...
						url: this.api.ApiRoot + "intentions/" + item.info.id + "/",
						data: {
							id: item.info.id,
							...
							wx_open_id: item.info.wx_open_id,
							wx_union_id: item.info.wx_union_id,
							wx_name: item.info.wx_name,
							wx_avatar_url: item.info.wx_avatar_url
						...
						success(res) {
							...
							uni.showModal({
								...
								success: function(res) {
									if (res.confirm) {
										uni.redirectTo({
											url: `/pages/intentions/intention_customer`
										})
									} else if (res.cancel) {
										uni.redirectTo({
											url: `/pages/intentions/intention_customer`
			...
			getIntentions() {
				return new Promise(resolve => {
					uni.request({
						...
						url: this.api.ApiRoot + "intentions/queryByUser/?wxid=" + uni.getStorageSync('userDetail').wx_open_id,
			...
			intentionflagsExp(flags) {
				...
				switch (+flags) {
					...
					case 0:
						flagsTip = '未分配';
						flagsTipColor = '#fa6c1d';
						break;
					case 1:
						flagsTip = '已分配';
						flagsTipColor = '#5efa54';
						break;
					case 2:
						flagsTip = '已完成';
						flagsTipColor = '#8c62fa';
						break;
					case 3:
						flagsTip = '已取消';
						flagsTipColor = '#000000';
						break;

					case 9:
						flagsTip = '订单已关闭';
						flagsTipColor = '#909399';
						flagsTipColor = '#f4faf9';
						break;
			...
			navToDetailPage(item) {
				...
				let msg = item;
				uni.navigateTo({
					url: `/pages/intention/intentionDetail_customer?data=${encodeURIComponent(JSON.stringify(msg))}`,
